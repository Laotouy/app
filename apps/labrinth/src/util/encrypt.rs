...
pub fn encrypt(plaintext: &str) -> Result<String, EncryptionError> {
    ...
    let unbound_key = UnboundKey::new(&AES_256_GCM, key_bytes)
        .map_err(|e| EncryptionError::EncryptionFailed(format!("创建密钥失败: {}", e)))?;
    ...
    rng.fill(&mut nonce_bytes)
        .map_err(|_| EncryptionError::EncryptionFailed("生成随机数失败".to_string()))?;
    ...
    key.seal_in_place_append_tag(nonce, Aad::empty(), &mut in_out)
        .map_err(|_| EncryptionError::EncryptionFailed("加密失败".to_string()))?;
...
pub fn decrypt(ciphertext: &str) -> Result<String, EncryptionError> {
    ...
    let data = BASE64
        .decode(ciphertext)
        .map_err(|e| EncryptionError::InvalidFormat(format!("Base64 解码失败: {}", e)))?;
    ...
    let nonce_array: [u8; 12] = nonce_bytes
        .try_into()
        .map_err(|_| EncryptionError::InvalidFormat("Nonce 长度错误".to_string()))?;

    let unbound_key = UnboundKey::new(&AES_256_GCM, key_bytes)
        .map_err(|e| EncryptionError::DecryptionFailed(format!("创建密钥失败: {}", e)))?;
    let key = LessSafeKey::new(unbound_key);
    ...
    let decrypted = key
        ...
        .map_err(|_| EncryptionError::DecryptionFailed("解密失败，密钥或数据错误".to_string()))?;
